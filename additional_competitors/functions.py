...
def get_preference(n):
  ...
  losers = [np.random.uniform(-0.01, 0.01) for _ in range(7)]
  if n < 0.5:
    ...
    return losers[:3] + [winner] + losers[3:]
  elif n < 1.05:
    return losers[:4] + [winner] + losers[4:]
  elif n < 1.10:
    return losers[:5] + [winner] + losers[5:]
  elif n < 1.15:
    return losers[:6] + [winner] + losers[6:]
  else:
